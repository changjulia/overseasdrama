...
export function MyCreations({ drafts, favorites = favoriteMocks, initialTab = "favorites", onContinueEdit, onReuseDraft, onUseFavorite, onOpenInspiration, onRemoveFavorite, onDraftChange, onRemoveDraft, onNotify }: MyCreationsProps) {
  ...
  const removeFavorite = (item: Favorite) => { if(!window.confirm(`确定从“我的创作”删除收藏素材“${item.title}”吗？\n\n灵感大屏中的源素材不会被删除。`))return;setLocalFavorites((items) => items.filter((x) => x.id !== item.id)); setOpenMenu(null); onRemoveFavorite?.(item.id); onNotify?.(`已删除收藏素材「${item.title}」`); };
  const removeDraft = (draft: Draft) => { if(!window.confirm(`确定删除创作草稿“${draft.title}”吗？\n\n草稿参数和当前版本记录将被移除，此操作不可撤销。`))return;setLocalDrafts((items) => items.filter((item) => item.id !== draft.id));onRemoveDraft?.(draft.id);setOpenMenu(null);onNotify?.(`已删除草稿「${draft.title}」`); };
      ...
      <div className={styles.favoriteGrid}>{visibleFavorites.map((item) => <article key={item.id}>
        ...
        <div className={styles.favoriteBody}><div className={styles.tags}><span>{item.language}</span><span>{item.theme}</span></div><h3>{item.title}</h3><p>{item.hook}</p><dl><div><dt>来源</dt><dd>{item.source}</dd></div><div><dt>收藏</dt><dd>{item.savedAt}</dd></div></dl>
          <div className={styles.favoriteActionBar}>
            ...
            <button type="button" className={styles.deleteAssetAction} onClick={()=>removeFavorite(item)}>删除</button><button type="button" className={`${styles.favoriteMore} ${openMenu === `favorite-${item.id}` ? styles.favoriteMoreActive : ""}`} aria-label={`更多操作：${item.title}`} aria-expanded={openMenu === `favorite-${item.id}`} onClick={() => setOpenMenu(openMenu === `favorite-${item.id}` ? null : `favorite-${item.id}`)}>•••</button>
