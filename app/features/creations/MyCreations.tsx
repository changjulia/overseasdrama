  ...
  return <section className={styles.creations} aria-label="我的创作">
    ...
    {tab === "favorites" ? <>
      ...
      <div className={styles.favoriteGrid}>{visibleFavorites.map((item) => <article key={item.id}>
        <div className={`${styles.preview} ${styles[item.tone]} ${playingFavorite === item.id ? styles.previewPlaying : ""}`}><span>{item.kind}</span>{item.previewUrl ? <video src={item.previewUrl} controls preload="metadata" onPlay={() => setPlayingFavorite(item.id)} onPause={() => setPlayingFavorite(null)} style={{width:"100%",height:"100%",objectFit:"cover"}} /> : <><button type="button" disabled aria-label={`暂无可播放文件：${item.title}`}>▶</button><em>未连接预览文件</em></>}</div>
      ...
      <div className={styles.draftTable}><div className={styles.tableHead}><span>草稿 / 剧目</span><span>模式与素材</span><span>输出规格</span><span>质检 / 生产状态</span><span>最近保存</span><span>操作</span></div>{visibleDrafts.map((draft) => <article key={draft.id}>
        <div className={styles.draftName}><i className={`${styles.draftThumb} ${styles[draft.thumbnailTone]}`}>{draft.thumbnailUrl ? <video src={`${draft.thumbnailUrl}#t=0.001`} muted playsInline preload="metadata" aria-label={`${draft.title} 首帧缩略图`} /> : <span>▶</span>}<em>{draft.duration}</em></i><span><b>{draft.title}</b><small>{draft.drama}</small></span></div>
