...
export type MaterialObservation = { factId:string; start:number; end:number; actorObserved:string; actionObserved:string; objectOrTargetObserved?:string; resultObserved?:string; verification?:string };
export type MaterialInference = MaterialTag & { statement:string; inferenceType?:string; basedOnFactIds:string[] };
...
export type MaterialAnalysisV2 = {
  ...
  creative: {
    ...
    hook?: { start?: number; end?: number; hookType?: string; source?: string; plotSummary?: string; spokenSummary?: string; visualSummary?: string; narrativePromise?: string; informationGap?: string; mechanisms: MaterialTag[]; sensoryChannels: MaterialTag[] };
...
function tags(value: unknown, dimension: OntologyDimension = "theme"): MaterialTag[] {
  return list(value).map((raw, index) => {
    ...
    return {
      ...
      confidence: item.confidence === undefined || item.confidence === null ? Number.NaN : number(item.confidence),
      evidence: rawEvidence.map(value => typeof value === "string" ? value : text(object(value).sourceText, text(object(value).text))).filter(Boolean),
      evidenceSources: rawEvidence.map(value => text(object(value).source)).filter(Boolean),
      verification: text(item.verification) || undefined,
    };
  });
}

const technicalThemePattern = /(?:语音|ASR|OCR|字幕|识别|置信度|混合语言|多语言内容|疑似(?:产品|专有|人物)名称|技术|画质|音质|分辨率|语种|翻译质量)/i;
const genericStoryBeatPattern = /^(?:第[一二三四五六七八九十\d]+段语义单元|语义段落(?::.*)?|主体叙事段落|叙事主体段落|核心叙事段落(?::.*)?|核心剧情(?:推进)?段|核心剧情段落(?::.*)?|重复引入段|心理对话段|经济情境段|互动收尾段|角色互动|对话驱动|(?:开场|再)?钩子|正片主体|主体段落|中期冲突|高潮(?:段落|与行动号召)?|悬念点|行动号召)$/u;
const narrativeFormThemePattern = /^(?:对话驱动|角色互动|任务管理|信息交换|剧情推进|叙事不完整)$/u;

function contentTags(value: unknown, dimension: OntologyDimension): MaterialTag[] {
  const seen = new Set<string>();
  return tags(value, dimension).filter(tag => {
    const label = tag.label.trim();
    if (!label || seen.has(label)) return false;
    if (dimension === "theme" && (technicalThemePattern.test(label) || narrativeFormThemePattern.test(label))) return false;
    if (dimension === "storyBeat" && genericStoryBeatPattern.test(label)) return false;
    seen.add(label);
    return true;
  }).map(tag => {
    if(tag.verification !== "verified")return tag;
    const sources=new Set((tag.evidenceSources??[]).map(value=>value.toLowerCase()));
    const hasTextEvidence=["transcript","asr","ocr","subtitle","manual_review"].some(source=>sources.has(source));
    const privilegedIdentity=/(?:queen|princess|emperor|king|prince|duke|doctor|ceo|billionaire|女王|公主|皇帝|国王|王子|公爵|医生|总裁|亿万富翁)/i.test(tag.label);
    const insufficient=!tag.evidence?.length||(dimension==="role"&&privilegedIdentity&&!hasTextEvidence)||(dimension==="relation"&&!hasTextEvidence);
...
function parseV2(value: unknown): MaterialAnalysisV2 | undefined {
  ...
  const semantic=object(candidate.semantic),semanticContent=object(semantic.content),semanticCreative=object(semantic.creative);
    ...
    const content={...semanticContent,...object(candidate.content)},creative={...semanticCreative,...object(candidate.creative)};
  ...
  const collectClaimEvidence=(value:unknown,parentClaim="")=>{
    if(Array.isArray(value)){for(const item of value)collectClaimEvidence(item,parentClaim);return}
    const item=object(value);if(!Object.keys(item).length)return;
    const ownClaim=text(item.label,text(item.value,text(item.description,parentClaim)));
    if(Array.isArray(item.evidence))for(const raw of item.evidence){const evidence=object(raw),source=text(evidence.source);if(source&&(evidence.timecode||evidence.start!==undefined))claimEvidence.push({raw,kind:source,supports:ownClaim||parentClaim})}
    for(const [key,child] of Object.entries(item))if(key!=="evidence"&&key!=="sourceText")collectClaimEvidence(child,ownClaim||parentClaim);
  };
  collectClaimEvidence(content,"剧情理解");collectClaimEvidence(creative,"创意结构");collectClaimEvidence(object(candidate.value),"投放价值");
  const translationByTime=new Map<string,string>();
  for(const item of translatedEvidence)translationByTime.set(`${item.source}|${Math.round(item.start*4)}|${Math.round(item.end*2)}`,item.text);
  const groupedEvidence = [
    ["transcript", "asr"], ["asr", "asr"], ["ocr", "ocr"], ["keyframes", "frame"], ["shots", "shot"], ["audioEvents", "audio"],
  ] as const;
  const semanticEvidence=list(content.segments).flatMap(segment=>{const item=object(segment);return list(item.evidence).map(raw=>{const evidence=object(raw);return{raw:{...evidence,kind:text(evidence.source,"segment"),translation:text(evidence.text),text:text(evidence.sourceText)},kind:text(evidence.source,"segment")}})});
  const rawEvidence = claimEvidence.length
    ? claimEvidence
    : semanticEvidence.length
    ? semanticEvidence.map(item=>({...item,supports:"剧情阶段"}))
    : Array.isArray(candidate.evidence)
      ? list(candidate.evidence).map(raw => ({ raw, kind: "frame",supports:"" }))
      : groupedEvidence.flatMap(([field, kind]) => list(evidenceGroup[field]).map(raw => ({ raw, kind,supports:"" })));
  const allEvidence = rawEvidence.map(({ raw, kind, supports }, index) => { const item = object(raw), timecode = object(item.timecode), shot = number(item.shot, -1), evidenceText=text(item.text, shot >= 0 ? `镜头 ${shot}` : text(item.type)),start=number(item.start,number(timecode.start)),end=number(item.end,number(timecode.end,start)),matched=translationByTime.get(`${evidenceGroupForStore(kind)}|${Math.round(start*4)}|${Math.round(end*2)}`),observation=text(item.translation,text(item.text,matched)),sourceText=text(item.sourceText); return {
    id: text(item.id, `evidence-${kind}-${index + 1}`), kind: text(item.kind, text(item.source, kind)),
    start, end,
    text: evidenceText || undefined, translation: text(item.translation,matched) || undefined,
    observation: observation || undefined, supports: supports ? supports.slice(0, 96) : undefined, sourceText: sourceText || undefined,
    evidenceClass: observation&&supports?"narrative" as const:observation?"auxiliary" as const:"technical" as const,
  ...
  const uniqueEvidence=allEvidence.filter(item=>{const key=`${item.kind}|${Math.round(item.start*20)}|${item.translation||item.text||""}`;if(evidenceKeys.has(key))return false;evidenceKeys.add(key);return true}).sort((left, right) => left.start - right.start || left.end - right.end);
  const evidenceLimit = 48;
  const evidence = uniqueEvidence.length <= evidenceLimit ? uniqueEvidence : Array.from({ length: evidenceLimit }, (_, index) => uniqueEvidence[Math.round(index * (uniqueEvidence.length - 1) / (evidenceLimit - 1))]);
  const hook = object(creative.hook),completeHook=object(list(creative.hooks)[0]),entryPoint=object(list(creative.entryPoints)[0]),hookSource=Object.keys(hook).length?hook:Object.keys(completeHook).length?completeHook:entryPoint,valuePart = object(candidate.value), review = object(candidate.review);
  const displayPhases=list(creative.displayPhases),creativeTimeline=list(creative.timeline),storyBeats=list(content.storyBeats),storyTimeline=list(content.segments);
  const genericStoryLabel=/^(?:(?:开场|再)?钩子|正片主体|主体段落|中期冲突|高潮(?:段落|与行动号召)?|悬念点|行动号召|CTA|剧情推进|未命名段落)$/i;
  const concreteStoryItems=(items:unknown[])=>items.filter(raw=>{const item=object(raw),label=text(item.label,text(item.value));return Boolean(label)&&!genericStoryLabel.test(label)});
  const splitNarrativeClauses=(value:string)=>{const clauses:string[]=[],opening=new Set(["“","‘","\""]),closing=new Set(["”","’","\""]);let buffer="",quote="";for(const char of value){if(opening.has(char)){if(char==='"'&&quote==='"')quote="";else if(!quote)quote=char;buffer+=char;continue}if(closing.has(char)&&quote&&char!==quote){quote="";buffer+=char;continue}if(!quote&&/[，；。]/u.test(char)){if(buffer.trim())clauses.push(buffer.trim());buffer="";continue}buffer+=char}if(buffer.trim())clauses.push(buffer.trim());return clauses};
  const completeSegmentTitle=(raw:unknown,index:number)=>{const item=object(raw),description=text(item.description).replace(/随后[，、：:]?\s*/gu,"").trim(),clauses=splitNarrativeClauses(description),first=clauses.find(value=>!/^为.+(?:而|以|，)?$/u.test(value))||clauses[0],outcome=[...clauses].reverse().find(value=>/(?:结果|关系|引发|导致|转向|升级|确认|认可|紧张|缓和|决裂|悬念|危机)/u.test(value))||clauses.at(-1),derived=[first,outcome&&outcome!==first?outcome:""].filter(Boolean).join("，").replace(/，(?:结果|关系变化为)/u,"，"),stored=text(item.label,text(item.value));return derived||stored||`剧情节点${index+1}`};
  const withCompleteTitles=(items:unknown[])=>items.map((raw,index)=>{const item=object(raw),label=completeSegmentTitle(item,index);return{...item,label,value:label}});
  const storyboardRaw=withCompleteTitles(list(content.storyboardUnits)),completeStoryboardBeats=concreteStoryItems(storyboardRaw),concreteBeats=completeStoryboardBeats.length?completeStoryboardBeats:[...concreteStoryItems(storyBeats),...concreteStoryItems(storyTimeline)];
  const observationBeats=list(content.observations).map((raw,index)=>{const item=object(raw),actor=text(item.actorObserved,"当事人"),action=text(item.actionObserved),result=text(item.resultObserved);return{code:text(item.factId,`OBSERVED_BEAT_${index+1}`),label:`${actor}${action}`.trim(),value:`${actor}${action}`.trim(),description:[action,result&&`结果：${result}`].filter(Boolean).join("；"),confidence:number(item.confidence,1),evidence:list(item.evidence),verification:text(item.verification,"unverified")}}).filter(item=>item.label&&item.label!=="当事人");
  const storyBeatSource=concreteBeats.length?concreteBeats:observationBeats;
  const rawScenes=concreteStoryItems(list(content.scenes));
  const inferredSceneLabels=rawScenes.length?[]:[...new Set(list(content.observations).flatMap(raw=>list(object(raw).evidence).map(evidence=>text(object(evidence).text)).flatMap(value=>value.match(/宫殿|宴会厅|卧室|医院|诊室|办公室|会议室|街头|餐厅|酒吧|舞蹈室|排练室|竞技场|森林|雪地|废墟|悬崖|庭院|广告界面/g)??[])))];
  const sceneSource=rawScenes.length?rawScenes:inferredSceneLabels.map((label,index)=>({code:`INFERRED_SCENE_${index+1}`,label,value:label,confidence:.7,verification:"observed"}));
  const hasConcreteDescription=(items:unknown[])=>items.some(raw=>text(object(raw).description).replace(/\s+/g,"").length>=8);
  ...
  const timelineSource=hasConcreteDescription(displayPhases)?displayPhases:hasConcreteDescription(storyTimeline)?storyTimeline:hasConcreteDescription(creativeTimeline)?creativeTimeline:hasConcreteDescription(storyBeats)?storyBeats:displayPhases.length?displayPhases:storyTimeline.length?storyTimeline:creativeTimeline.length?creativeTimeline:storyBeats;
  const parseSegments=(source:unknown[],prefix:string)=>source.map((raw,index)=>{const item=object(raw);return{code:text(item.code,`${prefix}_${index+1}`),label:completeSegmentTitle(item,index),start:number(item.start),end:number(item.end),description:text(item.description)||undefined,confidence:number(item.confidence),evidence:list(item.evidence).map(value=>text(object(value).text,text(value))).filter(Boolean)}});
  const timeline = parseSegments(timelineSource,"SEGMENT");
  const storyboardUnits=parseSegments(storyboardRaw,"STORYBOARD");
  const packagingObject=object(creative.packaging),packagingValues=[...list(packagingObject.visual),...list(packagingObject.subtitle),...list(packagingObject.audio),...list(packagingObject.rhythm)];
  ...
  return {
    ...
    content: { summary: claimTextValue(content.summary) || undefined, completeness: claimTextValue(content.completeness) || undefined,
      observations:list(content.observations).map((raw,index)=>{const item=object(raw);return{factId:text(item.factId,`F${index+1}`),start:number(item.start,number(object(item.timecode).start)),end:number(item.end,number(object(item.timecode).end)),actorObserved:text(item.actorObserved,"未知主体"),actionObserved:text(item.actionObserved,"缺少可观察动作"),objectOrTargetObserved:text(item.objectOrTargetObserved)||undefined,resultObserved:text(item.resultObserved)||undefined,verification:text(item.verification)||undefined}}),
      inferences:tags(content.inferences).map((tag,index)=>{const item=object(list(content.inferences)[index]);return{...tag,statement:text(item.statement,tag.label),inferenceType:text(item.inferenceType)||undefined,basedOnFactIds:list(item.basedOnFactIds).filter((value):value is string=>typeof value==="string")}}),
      genres: contentTags(content.genres, "genre"), themes: contentTags(content.themes??content.tags, "theme"), characters: contentTags(content.characters, "role"), relations: contentTags(content.relations??content.relationships, "relation"), emotions: contentTags(content.emotions, "emotion"), conflicts: contentTags(content.conflicts, "conflict"), storyBeats: contentTags(storyBeatSource, "storyBeat"), scenes: contentTags(sceneSource, "scene"), storyboardUnits },
    creative: { materialType: tag(creative.materialType??creative.format), tLevel: tag(creative.tLevel??creative.tier), bodyFormat: tag(creative.bodyFormat), hookSourceStatus: tag(creative.hookSourceStatus), hookAssemblyType: tag(creative.hookAssemblyType), narrationCoverage: number(object(creative.narrationCoverage).value, number(creative.narrationCoverage)), hook: Object.keys(hookSource).length ? { start: number(hookSource.start), end: number(hookSource.end), hookType: text(hookSource.hookType,text(hookSource.type,text(hookSource.label))) || undefined, source: text(hookSource.plotSummary,text(hookSource.spokenSummary,text(hookSource.label))) || undefined, plotSummary: text(hookSource.plotSummary) || undefined, spokenSummary: text(hookSource.spokenSummary) || undefined, visualSummary: text(hookSource.visualSummary) || undefined, narrativePromise: text(hookSource.narrativePromise) || undefined, informationGap: text(hookSource.informationGap) || undefined, mechanisms: tags(hookSource.mechanisms??hookSource.contentTags), sensoryChannels: tags(hookSource.sensoryChannels) } : undefined, timeline, packaging: tags(packagingValues), transitions: tags(creative.transitions) },
...
export function fromRecord(record: PBRecord, job?: PBRecord): InspirationMaterial {
  ...
  return {
    ...
    media: video || text(record.source_url) ? {
    ...
    coverUrl: fileUrl(record, cover) || `/material-covers/${encodeURIComponent(record.id)}.webp`,
    createdAt,
...
export async function listInspirationMaterials(signal?: AbortSignal): Promise<InspirationMaterial[]> {
  const materialFields = [
    "id","collectionId","collectionName","title","original_name","type","source","platform","market","language","theme",
    "exposure","days","created","review_status","analysis_status","analysis_progress","analysis_stage","analysis_error",
    "material_format","prototype","video","cover","mime_type","byte_size","duration_seconds","content_hash","source_url","rights_status","analysis_result",
  ].join(",");
  try {
    const [materialResponse, jobResponse] = await Promise.all([
      pbFetch(`/api/collections/ad_materials/records?perPage=500&sort=-id&fields=${materialFields}`, { signal }),
      pbFetch("/api/collections/material_analysis_jobs/records?perPage=500&sort=-id&fields=id,material,status,progress,logs", { signal }),
    ]);
    ...
    const cached = await fetch("/material-analysis/index.json", { signal, cache: "no-store" });
    if (!cached.ok) throw reason;
    return await cached.json() as InspirationMaterial[];
  }
}

export type InspirationMaterialPage = { items: InspirationMaterial[]; page: number; perPage: number; totalItems: number; totalPages: number };

export async function listInspirationMaterialsPage(page=1, perPage=24, signal?:AbortSignal):Promise<InspirationMaterialPage>{
  const materialFields = [
    "id","collectionId","collectionName","title","original_name","type","source","platform","market","language","theme",
    "exposure","days","created","review_status","analysis_status","analysis_progress","analysis_stage","analysis_error",
    "material_format","prototype","video","cover","mime_type","byte_size","duration_seconds","content_hash","source_url","rights_status","analysis_result",
  ].join(",");
  const boundedPage=Math.max(1,Math.floor(page)),boundedPerPage=Math.max(1,Math.min(60,Math.floor(perPage)));
  const response=await pbFetch(`/api/collections/ad_materials/records?page=${boundedPage}&perPage=${boundedPerPage}&sort=-id&fields=${materialFields}`,{signal});
  const payload=await response.json() as {items?:PBRecord[];page?:number;perPage?:number;totalItems?:number;totalPages?:number};
  const records=payload.items??[];
  const ids=records.map(item=>item.id).filter(Boolean);
  let jobs:PBRecord[]=[];
  if(ids.length){
    const filter=encodeURIComponent(ids.map(id=>`material="${id}"`).join(" || "));
    const jobResponse=await pbFetch(`/api/collections/material_analysis_jobs/records?perPage=${Math.min(200,ids.length*3)}&sort=-id&filter=${filter}&fields=id,material,status,progress,logs,error,error_kind`,{signal});
    jobs=((await jobResponse.json()) as {items?:PBRecord[]}).items??[];
  }
  const jobByMaterial=new Map<string,PBRecord>();
  for(const job of jobs)if(!jobByMaterial.has(text(job.material)))jobByMaterial.set(text(job.material),job);
  return {items:records.map(record=>fromRecord(record,jobByMaterial.get(record.id))),page:payload.page??boundedPage,perPage:payload.perPage??boundedPerPage,totalItems:payload.totalItems??records.length,totalPages:payload.totalPages??1};
}

export async function getInspirationMaterial(id: string, signal?: AbortSignal): Promise<InspirationMaterial> {
  try {
    const filter = encodeURIComponent(`material="${id.replace(/"/g, '\\"')}"`);
    const [materialResponse, jobResponse] = await Promise.all([
      pbFetch(`/api/collections/ad_materials/records/${encodeURIComponent(id)}`, { signal }),
      pbFetch(`/api/collections/material_analysis_jobs/records?perPage=1&sort=-id&filter=${filter}&fields=id,material,status,progress,logs`, { signal }),
    ]);
    ...
    const cached = await fetch(`/material-analysis/${encodeURIComponent(id)}.json`, { signal, cache: "no-store" });
    if (!cached.ok) throw reason;
    return await cached.json() as InspirationMaterial;
  }
}
...
export async function saveInspirationMaterial(input: InspirationMaterialInput | InspirationMaterial, video: File, onProgress?: (percent:number)=>void, queueAnalysis = true): Promise<InspirationMaterial> {
...
export async function saveExternalInspirationReference(input: ExternalInspirationReferenceInput, autoAnalyze = false): Promise<{material:InspirationMaterial;created:boolean;analysisQueued:boolean}> {
  const canonicalSource=(()=>{try{const url=new URL(input.sourceUrl);return `${url.origin}${url.pathname}`}catch{return input.sourceUrl}})();
  const dedupeSeed=`adx:${input.externalId||canonicalSource}`;
  const digest=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(dedupeSeed));
  const sourceIdentityHash=Array.from(new Uint8Array(digest),byte=>byte.toString(16).padStart(2,"0")).join("");
  ...
  const existingFilter=encodeURIComponent(`source_identity_hash="${sourceIdentityHash}" || content_hash="${sourceIdentityHash}" || source_url="${input.sourceUrl.replace(/"/g,'\\"')}"`);
  const existingResponse=await pbFetch(`/api/collections/ad_materials/records?perPage=1&filter=${existingFilter}`);
  const existing=await existingResponse.json() as {items?:PBRecord[]};
  if(existing.items?.[0]){
    const material=fromRecord(existing.items[0]);
    if(autoAnalyze&&!["queued","running","succeeded"].includes(material.analysisStatus??"idle")){
      await retryInspirationMaterialAnalysis(material.id);
      return {material:{...material,analysisStatus:"queued",analysisProgress:0,analysisStage:"等待素材 Worker",analysisError:undefined},created:false,analysisQueued:true};
    }
    return {material,created:false,analysisQueued:false};
  }
  const response=await fetch("/api/material-intake",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...input,sourceIdentityHash,autoAnalyze})});
  const payload=await response.json() as {record?:PBRecord;created?:boolean;analysisQueued?:boolean;message?:string};
  if(!response.ok||!payload.record)throw new Error(payload.message||`服务端入库失败（HTTP ${response.status}）`);
  return {material:fromRecord(payload.record),created:payload.created===true,analysisQueued:payload.analysisQueued===true};
}
...
export async function hashInspirationVideo(file:File):Promise<string>{
  if(typeof Worker!=="undefined")return await new Promise<string>((resolve,reject)=>{const worker=new Worker("/material-hash-worker.js");worker.onmessage=event=>{worker.terminate();event.data?.hash?resolve(event.data.hash):reject(new Error(event.data?.error||"视频哈希失败"))};worker.onerror=event=>{worker.terminate();reject(new Error(event.message||"视频哈希 Worker 失败"))};worker.postMessage(file)});
...
export async function retryInspirationMaterialAnalysis(id: string, force = false): Promise<void> {
