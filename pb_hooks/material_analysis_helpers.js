...
function sourceAttributionObject(value) {
  let current = value;
  for (let depth = 0; depth < 3; depth += 1) {
    if (current === null || current === undefined) return {};
    if (typeof current === "string") {
      try { current = JSON.parse(current); } catch (_) { return {}; }
      continue;
    }
    ...
    try {
      if (typeof current.toJSON === "function") {
        ...
        if (serialized !== current) { current = serialized; continue; }
      }
...
function mergeSourceAttribution(existingValue, analysisValue) {
  const existing = sourceAttributionObject(existingValue);
  if (existing.schema !== "csv-hook-reference-v1") {
    if (analysisValue === null || analysisValue === undefined) return analysisValue;
    return sourceAttributionObject(analysisValue);
  }
  const merged = sourceAttributionObject(existing);
  const analysis = sourceAttributionObject(analysisValue);
  delete analysis.analysisAttribution;
  merged.analysisAttribution = analysis;
  return merged;
}
...
module.exports = { appendRetryLineage, authorize, authorizeLocalUi, createJob, resultValue, projectMaterialResult, normalizeStage, sourceAttributionObject, mergeSourceAttribution, resetMaterialPublishedAnalysis, syncMaterialHookAssets };
